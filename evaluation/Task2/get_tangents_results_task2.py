...
def main():
    """
    example: python3 get_tangents_results_task1.py -res "slt-opt-combined.tsv"
    ...
    for topic_id in arq_math_result:
        ...
        for formula_id in retrieval_results:
            csv_writer.writerow(
                [str(topic_id), str(formula_id), str(dic_formula_id_to_answer_id[formula_id]), str(rank),
                 str(retrieval_results[formula_id]), "Tangent_S"])
            rank += 1
            if rank > 1001:
                break
