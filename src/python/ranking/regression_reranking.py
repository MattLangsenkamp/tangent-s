...
def load_topic_groups(groups_filename):
    input_file = open(groups_filename, "r")
    input_lines = input_file.readlines()
    input_file.close()

    groups = []
    for line in input_lines:
        queries = [val.strip() for val in line.split(",")]
        groups.append(queries)

    return groups

def separate_training_data(combined_results, qrels, topic_groups):
    folds_data = []
    folds_labels = []
    count_nonjudged = {}

    # for each group of topics
    for topic_group in topic_groups:
        group_features = []
        group_labels = []

        # get all training features for judged documents
        for query_id in topic_group:
            # for each formula for this query ....
            for formula_id in combined_results[query_id]:
                # check if it has a relevance judgment
                if formula_id in qrels[query_id]:
                    # it has, use for training ...
                    group_features.append(combined_results[query_id][formula_id])
                    group_labels.append(qrels[query_id][formula_id])
                else:
                    if not query_id in count_nonjudged:
                        count_nonjudged[query_id] = 1
                    else:
                        count_nonjudged[query_id] += 1

        folds_data.append(group_features)
        folds_labels.append(group_labels)

    return folds_data, folds_labels, count_nonjudged
...
def main():
    if len(sys.argv) < 9:
        print("Usage")
        print("    python3 regression_reranking.py combined_input qrels_agg topic_groups output_results")
        print("                                    output_graphs run_name best_only max_k [fields]")
        print("")
        ...
        print("\ttopic_groups:\tPath to file containing groups used for cross-validation")
        print("\t\t\t (CSV file containing one group per line)")
        print("\toutput_results:\tPath to store cross-validated testing results")
        print("\toutput_graphs:\tPath to directory where visualizations will be saved")
        print("\trun_name:\tName for output run")
        print("\tbest_only:\tOutput only best match per document")
    ...
    judged_qrels_agg_filename = sys.argv[2]
    topics_groups_filename = sys.argv[3]
    output_filename = sys.argv[4]
    graph_dir = sys.argv[5]
    run_name = sys.argv[6]

    try:
        best_only = int(sys.argv[7]) > 0
    except:
        print("Invalid value for best_only")
        return

    try:
        max_k = int(sys.argv[8])
    except:
    ...
    if len(sys.argv) > 9:
        try:
            chosen_fields = [int(field) for field in sys.argv[9::2]]
            field_names = [field_name for field_name in sys.argv[10::2]]

            if len(chosen_fields) != len(field_names):
                print("Names must be specified for each selected field")
                return
        except:
            print("Invalid values for field")
            return
    else:
        chosen_fields = None
        field_names = None
